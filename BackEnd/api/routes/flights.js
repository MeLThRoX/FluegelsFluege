...
const { authRequired, adminRequired, convertToMongoId } = require('../auth')
...
router.post('/read', authRequired, [
    check('_id').optional().isMongoId().customSanitizer(convertToMongoId).withMessage('Invalid ID Format'),
...
router.post('/book', authRequired, [
    check('flight_id').optional().isMongoId().customSanitizer(convertToMongoId).withMessage('Invalid Flight-ID Format'),
