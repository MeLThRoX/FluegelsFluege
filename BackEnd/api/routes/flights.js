...
router.post('/read', authRequired, [
    check('_id').optional().isMongoId().customSanitizer(v => check('id').isMongoId() ? undefined : ObjectId(v) ).withMessage('Invalid ID Format'),
...
router.post('/book', authRequired, [
    check('flight_id').optional().isMongoId().customSanitizer(v => check('id').isMongoId() ? undefined : ObjectId(v) ).withMessage('Invalid Flight-ID Format'),
